...
unsigned long long int reconstruct_object(unsigned char* byteImage) {
    if (!byteImage) {
        exit(EXIT_FAILURE);
    }
    unsigned long long int byteIndex = 0;
...
void write_image_to_file(const char* output_file, unsigned char* byteImage, unsigned long long int imageSize) {
